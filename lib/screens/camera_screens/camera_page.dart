...
class _CameraPageState extends State<CameraPage> {
    ...
    Navigator.push(
      ...
      MaterialPageRoute(
        builder: (context) => SetResultPage(
          ...
          slope: 0.0,
          yIntercept: 0.0,
          exerciseId: widget.exerciseId, oneRM: widget.oneRM, // 수정: maxSpeeds 전달
        ),
      ),
    ).then((_) {
      setState(() {
  ...
  void _onButtonPressed() async {
    if (_buttonText == '측정 시작') {
      ...
      int currentWeight = widget.testWeights[setNumber - 1];
      double currentSpeed =
